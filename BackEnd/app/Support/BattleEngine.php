...
class BattleEngine
{
     ...
     * 행동하는(자기 턴을 소모한) 유닛은 게이지를 0으로 리셋하고(사용자 지시 - 초과분
     * 이월 없음), 나머지 유닛도 이번 틱까지 쌓인 게이지를 그대로 저장해 다음 advanceTurn() 호출에서
    ...
    private function pickReadyActor(Collection $living): BattleUnit
    {
        ...
        // 선택되지 못한 유닛의 게이지를 100에 고정해봤더니(직전 커밋) 오히려 더 나빠졌다:
        // 대기 중인 유닛들의 점수(gauge*1000+spd)가 더 이상 안 자라니 spd가 낮은
        // 유닛은 나중에 새로 100을 넘긴 유닛에게 계속 밀려 영영 못 뽑히고, 그 뒤로
        // 매 advanceTurn()의 틱 루프가 "이미 대기 중인 유닛이 있다"는 이유로 1틱만
        // 돌고 바로 끝나버려서 나머지 유닛들도 100 도달이 느려지며 대기열만 계속
        // 불어나 결국 전원이 100%로 보이는 정체가 발생했다(실서버에서 확인). 그래서
        // 원래대로 되돌림: 선택 안 된 유닛은 이번 틱까지 쌓인 값(100 넘겨도 그대로)을
        // 저장한다 - 값이 계속 자라야 점수도 같이 올라가서 언젠가 반드시 뽑히는
        // 공정성이 보장된다.
        //
        // 행동한(자기 턴을 소모한) 유닛은 -100(초과분 이월)이 아니라 그냥 0으로
        // 리셋한다(사용자 지시) - 대기 유닛의 "계속 자라야 하는" 위 공정성 로직과는
        // 무관, 이미 뽑혀서 행동한 유닛 본인 얘기라 겹치지 않는다.
        foreach ($living as $u) {
            $u->atb_gauge = $u->id === $actor->id ? 0 : $gauges[$u->id];
            $u->save();
        }
    ...
    private function log(
        ...
        BattleLog::create([
            ...
            'skill_animation_id' => $skill?->tags['skill_animation_id'] ?? null,
    ...
    public function getState(Battle $battle): array
    {
        ...
        return [
            ...
            'logs' => $battle->logs()->get()->map(fn (BattleLog $l) => [
                ...
                'skill_animation_id' => $l->skill_animation_id,
